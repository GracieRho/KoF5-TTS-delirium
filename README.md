...
`http://127.0.0.1:8765/hospital`은 **병원 직원 조회 웹 초안**입니다. 같은 전용 Supabase 설정으로 Auth에 로그인한 뒤 호출자 RLS가 적용된 `api.hospital_patient_list`, `api.hospital_context_current`, `api.hospital_message_list`만 읽습니다. 현재 입원이 없는 환자의 병원 사실·메시지는 조회하지 않고, 승인 메시지 원문과 전달 상태·예정 시각을 한국 시간으로 표시합니다. 토큰은 페이지 메모리에만 보관하고 로그아웃 시 환자 상세 텍스트도 지웁니다. 작업 전용 로컬 합성 직원 계정에서 기관별 조회·자격 철회 차단을 HTTP로 확인했고 모바일 폭에서 가로 넘침을 확인했지만, 실제 기관 직원 자격 검증·환자 등록·승인·전달은 아직 연결되지 않았습니다.
