...
고정 합성 환자의 일반 가족 기억을 저장한 뒤, 보호자는 **후속 질문 보기**를 직접 누를 수 있습니다. 이때 서버는 보호자 연결·기억을 다시 확인해 시험용 LLM 후보에 그 기억 원문을 보내고, 결과를 반환하기 직전에 수정·철회를 다시 검사합니다. 질문은 검토용 표시만 하며 작성 중인 기억을 지우거나 자동 저장하지 않습니다. 보호자 화면은 원문이 시험용 공급자에게 전송됨을 클릭 전에 안내합니다. 모의 API/웹 검사만 통과했고 실제 공급자·실제 가족 정보 전송, 질문 품질과 보존 정책은 미검증입니다([PRD 기억 등록](docs/delirium-familiar-voice/03-context-and-guardian.md)).
...
병원 화면의 별도 합성 맥락 시험에서는 고정 합성 환자·현재 입원에 배정된 직원 A가 짧은 원문과 `TEST-*` 출처를 제안하고 **다른 담당 직원 B**가 승인해야만 기존 병원 사실 조회와 기기 질문 경로에 나타납니다. 병원 직원은 승인 사실의 합성 출처를 볼 수 있지만 기기 병원 질문은 출처·직원 ID를 받지 않습니다. 자기 승인·철회·기관/입원/동의 준비 부족은 DB에서 차단하며, 원문·출처는 승인 뒤 바꿀 수 없습니다([ADR-0013](architecture/decisions/0013-synthetic-hospital-context-two-staff-approval.md)). 실제 EHR 출처·임상 승인과 사실의 유효기간 정책은 기관 검토가 필요합니다.

합성 병원 메시지는 고정 합성 환자·현재 입원·세 동의·활성 음성 프로필이 모두 준비된 경우에만 담당 직원이 초안을 쓰고 **다른 담당 직원**이 승인해 예약 큐에 넣습니다. 기기는 익명 JWT로 기한이 된 메시지 ID 최대 세 개를 찾고, 개별 RPC에서 권한과 승인 원문을 다시 확인합니다. `POST /internal/synthetic/paired/{patient_id}/message/{message_id}/audio`는 원문을 언어 모델로 다시 쓰지 않고 시험 TTS에 그대로 보냅니다. iPad는 RPC 원문과 API 원문이 완전히 일치할 때만 재생하며, 재생 중단·철회 시 폐기합니다. 별도 합성 시험 스위치를 켜면 전면 iPad가 30초마다 대기 ID를 확인하고, 원문 재확인·마이크 중단·네이티브 재생 완료 뒤에만 기기 JWT로 같은 시도 ID의 DB 완료 RPC를 요청합니다. 빈 조회에서는 듣기를 유지하고 불확실한 DB 응답에 재생을 반복하지 않습니다. 병원 화면의 `delivered`는 이 경로에서 **iPad 재생 완료 보고**이며 실제 환자 청취·임상 전달이나 의료진 확인은 아닙니다([ADR-0012](architecture/decisions/0012-synthetic-hospital-message-device-playback-receipt.md)). 앱 재시작 뒤 중복 방지, 실제 환자·실기기·실제 공급자와 임상 원문 승인·전달은 아직 검증하지 않았습니다.
...
루트 `app.py`와 `vercel.json`은 [Vercel FastAPI 진입점](https://vercel.com/docs/frameworks/backend/fastapi) 및 Python 함수 번들 제외 설정입니다. 현재 **합성 텍스트 API와 인증된 내부 오디오 시험 API의 배포 준비** 단계입니다. 고정 합성 환자의 연결된 iPad 글 경로는 [현재 입원 DB 세션](architecture/decisions/0014-synthetic-admission-conversation-session.md)으로 두 번째 발화의 활성화 상태를 요청 간 유지한다. 다른 내부 데모의 메모리 세션은 함수 인스턴스 간 공유되지 않는다. DB 세션도 이전 LLM 턴 문맥·iPad 재생 종료 시각을 유지하지 않으며, 실제 환자 서비스와 Vercel 배포는 아직 검증하지 않았다.
