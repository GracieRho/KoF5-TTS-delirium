...
1. 병원 정보는 직원 승인·검증 시각·유효 기간을 가진 `hospital_context_fact`로 가족 기억과 분리한다. 읽기 뷰에는 활성 환자·현재 입원·승인된 유효 사실만 포함한다.
...
3. 병원 직원은 확인된 기관·담당 환자 범위에서만 사실과 메시지를 읽는다. 클라이언트가 직접 생성·승인·전달 상태를 쓰는 권한은 열지 않는다. 실제 승인·취소·전달·감사는 기관 권한 검증과 환자 동의/거부 절차를 갖춘 서버 흐름에서 구현한다.

## 결과와 검증

- 작업 전용 Supabase PostgreSQL 17의 [마이그레이션](../../supabase/migrations/20260915133351_hospital_context_messages.sql)에서 사실·메시지 RLS, 다른 환자 입원 참조 차단, 승인 메시지 불변성을 적용했다. 합성 pgTAP 19개와 전체 권한 검사 51개가 통과했고 advisor는 문제를 보고하지 않았다.
