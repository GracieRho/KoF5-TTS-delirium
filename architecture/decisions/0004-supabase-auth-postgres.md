...
- [네 로컬 마이그레이션](../../supabase/README.md)은 작업 전용 Supabase PostgreSQL 17에 적용했고, 합성 환자 제약 검사와 직원·보호자·병원 콘텐츠 pgTAP 51개가 통과했다. 로컬 GoTrue/Data API HTTP에서 합성 계정의 로그인·가족 기억 처리·철회 차단도 확인했다. 로컬 advisor는 문제를 보고하지 않았다. 전용 원격 프로젝트와 실제 기관 승인·환자 처리 흐름은 아직 검증하지 않았다.
