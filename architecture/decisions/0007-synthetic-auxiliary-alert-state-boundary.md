...
2. `created`는 DB 생성, `delivered`는 담당 직원의 합성 포털 표시 수신 버튼, `acknowledged`는 담당 직원의 명시 확인, `resolved`는 후속 조치 완료 표시, `failed`는 포털 표시/직원 확인 시간 초과다. 포털 조회만으로 전달 상태를 바꾸지 않는다. 시간 초과 계산은 담당 직원의 조회·동작 또는 명시적 sweep 때만 실행한다.
