...
- `apps/patient_ipad/`: Flutter iPad 클라이언트. 후보 발화는 기본적으로 기기에서 폐기합니다. 시험자 본인의 수동 오디오 전송과 기기 내 전사 후 글 전용 내부 시험을 분리합니다. 응답 재생 뒤 듣기를 재개하고, 재생 중 끼어들기 실험의 전사 글은 본인 목소리 확인을 거쳐서만 보냅니다. 실제 환자·화자 판정·자동 끼어들기 경로는 연결하지 않았습니다.
- `supabase/`: Auth/Postgres의 로컬 마이그레이션과 합성 권한 검사. 환자·입원·동의·음성 메타데이터, 가족 기억, 병원 승인 사실·메시지는 비공개 `kof5` 스키마에 두고, 검증된 직원·보호자 범위의 조회, 보호자 기억 등록·수정, 승인 게이트를 통과한 등록자만 환자 기본정보 입력을 호출자 RLS `api` 뷰로 노출합니다. 작업 전용 로컬 Auth/Data API의 합성 권한 시험은 통과했지만, 전용 원격 프로젝트·실제 기관 승인/메시지 전달·환자 자료·음성 저장소는 연결하지 않았습니다.
