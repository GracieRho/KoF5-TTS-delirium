...
| Audit | 최소 안전 이벤트 저장 테이블과 앱 역할 접근 차단. 합성 보조 알림의 상태를 제한된 이벤트로 기록하고, 합성 대화의 환자 거부는 세션 중단과 같은 트랜잭션에서 기존 `patient_dissent` 이벤트에 기록한다. [audit migration](../../supabase/migrations/20260915155000_phase5_audit_events.sql), [합성 세션](../../supabase/migrations/20260915220107_synthetic_conversation_session_cas.sql) | 실제 환자·임상 이벤트와 접근/삭제 이력의 승인된 기록자·조회자, 보존 기간과 기관 감사 검토 |
| Deletion | iPad 합성 후보는 휘발성 메모리에 두고 시험 중단 시 폐기. 환자 음성 프로필 철회·삭제의 재활성화는 차단한다. 합성 보호자 clone은 별도 동의·원격 목록 부재 후 `deleted` 전이를 기록하되 불확실하면 `deletion_pending`을 유지한다([clone lifecycle](../../supabase/migrations/20260916143000_synthetic_guardian_voice_lifecycle.sql)). | 원본·특징 객체·복제 음성·전사·가족 기억의 승인된 삭제 트랜잭션, 공급자 내부 보존/백업·샘플 삭제 확인, 실패 재시도와 감사 기록. API 목록 부재만으로 모든 내부 자료 삭제를 증명할 수 없음 |
...
현재 제품 경로도 합성 시험에 한정된다. 병원 웹은 검증된 등록자와 **기본 0행** 기관 승인 게이트가 있을 때에만 최소 환자 등록을 허용하며, 실제 승인·EHR·입원·동의·환자 목소리 등록 업무는 없다([병원 경계](../../supabase/README.md)). 별도 합성 병원 사실 경로는 결속된 기기·현재 입원·승인된 안전 범주·질문을 한 DB 조회로 대조하고, 모호한 결과에는 사실을 내지 않는다([병원 사실 경계](../../supabase/migrations/20260915183905_patient_hospital_turn_context.sql)). iPad는 자기 음성의 기기 내 전사와 글 시험, 고정 합성 환자 UUID에 대한 익명 기기 결속을 지원하지만 실기기 병실 화자 판정, 환자 음성 등록, 실제 보호자 복제 음성은 검증되지 않았다([iPad 범위](../../apps/patient_ipad/README.md)). 보호자 웹의 합성 일반 가족 기억은 저장 확인 뒤 임시 의미 색인을 요청하고, 결속된 기기는 현재 권한·동의를 다시 확인해 최대 세 건을 검색한다([맥락 현황](03-context-and-guardian.md)). 로컬 pgTAP과 Auth/Data API 경계는 통과했지만 실제 한국어 의미 검색 품질·오검색·임상 내용 안전성은 미측정이다.
...
같은 입원의 다회차 활성화 상태는 [입원별 DB 세션](../../supabase/migrations/20260915220107_synthetic_conversation_session_cas.sql)에서 ID·버전 비교 후 갱신한다. 합성 API는 별도 요청의 두 번째 `UNCERTAIN` 발화도 활성 대화로 처리하고, 공급자 임베딩 전·답변/TTS 직전에 현재 세션·권한을 다시 확인한다([ADR-0014 제안](../../architecture/decisions/0014-synthetic-admission-conversation-session.md)). 세션에는 환자 글·오디오를 저장하지 않고 불확실한 후속 발화를 검증된 직접 전사로 기록하지 않는다. 45초 만료는 DB 갱신부터 시작해 실제 음성 재생 종료 뒤 무음과 다르며, 이전 LLM 대화 문맥·전체 턴 ID 멱등·실기기 화자 판정은 구현되지 않았다. 이미 시작한 공급자 요청을 철회 시점에 취소하는 보장도 없다.

연결된 합성 글 경로는 현재 동의·입원·보호자 연결을 확인해 유효한 시험 clone이 정확히 하나일 때만 TTS voice ID를 선택한다([음성 선택](../../supabase/migrations/20260916163000_synthetic_patient_tts_voice_selection.sql)). 받아들인 `DIRECTED` 글은 기기 권한으로 임베딩/LLM/TTS 전에 기록하고, 담당 직원만 현재 입원의 한국 날짜 오늘 글을 조회한다([전사 경계](../../supabase/migrations/20260916170000_synthetic_directed_transcript.sql), [ADR-0011 제안](../../architecture/decisions/0011-synthetic-directed-transcript-today-view.md)). 합성 예약 병원 메시지는 별도 iPad 스위치가 켜진 전면 상태에서만 기한 된 ID를 찾고, 네이티브 재생 완료 뒤 기기 JWT로 DB 완료를 보고한다([ADR-0012](../../architecture/decisions/0012-synthetic-hospital-message-device-playback-receipt.md)). 합성 병원 사실은 담당 직원 A의 원문·`TEST-*` 출처 제안과 다른 담당 직원 B의 승인을 거쳐야 현재 입원 기기 질문에 노출된다([ADR-0013](../../architecture/decisions/0013-synthetic-hospital-context-two-staff-approval.md)). 깨끗한 로컬 DB의 pgTAP **558개/19파일**과 Auth/Data API 선택·기록·조회·철회·예약 완료/멱등·병원 사실 승인·입원 세션 CAS/거부 경합 검사가 통과했다. 실제 로컬 FastAPI 두 턴은 합성 시험 clone 메타데이터와 **모의 TTS**로만 연결했으며, 실제 보호자 음성 생성/재생, 실기기 화자 판정·첫 청취 지연, 원격 배포·기관 승인 및 자동 전사 삭제는 검증되지 않았다.
