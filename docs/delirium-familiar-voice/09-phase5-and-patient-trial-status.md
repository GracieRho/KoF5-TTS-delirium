...
| Audit | 9개 안전 이벤트 유형의 저장 테이블과 앱 역할 접근 차단. [audit migration](../../supabase/migrations/20260915155000_phase5_audit_events.sql) | 승인된 기록자·조회자, 실제 이벤트 기록 경로, 접근/삭제 이력, 보존 기간과 기관 감사 검토 |
...
| Failure handling | 내부 API의 권한/입력/공급자 오류, iPad 마이크·전사·재생 중단 확인 실패 시 차단, 기기 권한 조회 실패 시 hosted 호출 중단. [API](../../src/kof5_tts/api.py), [iPad 경계](../../apps/patient_ipad/README.md) | 실제 병실 네트워크·기기·공급자 장애 시험, 임상 경고 전달/ACK/timeout/fallback 및 환자 안내 검증 |
...
현재 제품 경로도 합성 시험에 한정된다. 병원 웹은 검증된 등록자와 **기본 0행** 기관 승인 게이트가 있을 때에만 최소 환자 등록을 허용하며, 실제 승인·EHR·입원·동의·환자 목소리 등록 업무는 없다([병원 경계](../../supabase/README.md)). iPad는 자기 음성의 기기 내 전사와 글 시험, 고정 합성 환자 UUID에 대한 익명 기기 결속을 지원하지만 실기기 병실 화자 판정, 환자 음성 등록, 실제 보호자 복제 음성은 검증되지 않았다([iPad 범위](../../apps/patient_ipad/README.md)). 보호자 웹의 가족 기억 등록·문자열 검색은 합성 계정에서 검사했으며 실제 가족 기억의 한국어 의미 검색 품질은 미측정이다([맥락 현황](03-context-and-guardian.md)).
...
| Clinical Escalation | **미완료** | 위험 발화 정의, 전달 성공·의료진 ACK·timeout·실패·기존 간호 호출 fallback의 폐루프 검증 |
