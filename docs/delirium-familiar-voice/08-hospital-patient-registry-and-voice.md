...
직원 승인 병원 사실과 예약 메시지는 [ADR-0005](../../architecture/decisions/0005-hospital-approved-facts-message-fidelity.md)에 따라 가족 기억에서 분리하며, 승인된 메시지 원문은 변경하지 않는다. 퇴원하면 병원 사실·메시지를 조회하지 않고, 재입원 시 이전 입원의 사실·메시지를 제외한다. 고정 합성 환자의 병원 사실은 [ADR-0013](../../architecture/decisions/0013-synthetic-hospital-context-two-staff-approval.md)의 두 담당 직원 원문·`TEST-*` 출처 승인 UI/DB 경로와 로컬 Auth/Data API 왕복을 검증했다. 실제 EHR 출처 확인·임상 문구 승인·전달은 아직 구현되지 않았다.
