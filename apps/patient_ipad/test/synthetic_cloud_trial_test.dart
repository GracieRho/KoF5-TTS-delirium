...
void main() {
  test('manual own-voice candidate reaches only the guarded WAV endpoint', () async {
    final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
    final seen = Completer<(String?, String?, List<int>)>();
    server.listen((request) async {
      final bytes = await request.fold<List<int>>(<int>[], (body, chunk) => body..addAll(chunk));
      seen.complete((
    ...
    try {
      ...
      expect(ByteData.sublistView(Uint8List.fromList(wav)).getUint32(40, Endian.little), 4);
  ...
  test('remote HTTP, wrong path and short token fail before sending audio', () async {
    final client = HttpClient();
    try {
